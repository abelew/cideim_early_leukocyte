...
labels <- as.character(pData(hs_macr)[["figurelabel"]])
...
pp(file="figures/fig_s2a_libsizes.pdf", image=fig_s1$raw_libsize)

pp(file="figures/fig_s2b_heatmap.pdf", image=fig_s1$norm_disheat)

hs_cdsn <- normalize_expt(hs_cds_macr, filter=TRUE, norm="quant",
                          convert="cpm", transform="log2")

pp(file="figures/fig_2sc_raw_pca.tif",
   image=plot_pca(hs_cdsn)$plot)

hs_cdsnb <- normalize_expt(hs_cds_macr, filter=TRUE, batch="svaseq",
                           convert="cpm", transform="log2")
hs_cdsnb_pca <- plot_pca(hs_cdsnb, label_size=6)
pp(file="figures/fig_4a_norm_pca.pdf", image=hs_cdsnb_pca$plot)

nb_disheat <- plot_disheat(hs_cdsnb, cexRow=1.6, cexCol=1.6)
pp(file="figures/fig_s2b_heatmap.pdf", image=nb_disheat$plot)
```
