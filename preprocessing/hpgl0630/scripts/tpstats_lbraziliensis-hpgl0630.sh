...
#PBS -N tpstats_lbraziliensis-hpgl0630 -l mem=1gb -l walltime=00:10:00 -l ncpus=1
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0630/outputs/qsub/tpstats_lbraziliensis-hpgl0630.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0630/scripts/tpstats_lbraziliensis-hpgl0630.sh at $(date)" >> outputs/log.txt
...
original_reads_tmp=$(grep "^Input Reads" outputs/hpgl0630-trimmed-trimomatic.out | awk '{print $3}' | sed 's/ //g')
...
rpm=${rpm_tmp:-0}
stat_string=$(printf "hpgl0630-trimmed,lbraziliensis,%s,%s,%s,%s,%s,accepted_hits.count.xz" "${original_reads}" "${reads}" "${aligned}" "${failed}" "$rpm")
echo "$stat_string" >> outputs/tophat_stats.csv

## The following lines give status codes and some logging
echo $? > outputs/status/tpstats_lbraziliensis-hpgl0630.status
echo "###Finished ${PBS_JODID} tpstats_lbraziliensis-hpgl0630.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
