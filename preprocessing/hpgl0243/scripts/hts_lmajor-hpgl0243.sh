...
#PBS -N hts_lmajor-hpgl0243 -l mem=6gb -l walltime=10:00:00 -l ncpus=4
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0243/outputs/qsub/hts_lmajor-hpgl0243.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0243/scripts/hts_lmajor-hpgl0243.sh at $(date)" >> outputs/log.txt
cd /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0243 || exit

## Counting the number of hits in outputs/tophat_lmajor/accepted_hits.bam for each feature found in /cbcbhomes/abelew/libraries/genome/lmajor.gff
...
htseq-count -q -f bam -s no  -i ID  \
  outputs/tophat_lmajor/accepted_hits.bam /cbcbhomes/abelew/libraries/genome/lmajor.gff \
  1>outputs/tophat_lmajor/accepted_hits.count 2>outputs/tophat_lmajor/accepted_hits.error && \
    xz -9e outputs/tophat_lmajor/accepted_hits.count

## The following lines give status codes and some logging
echo $? > outputs/status/hts_lmajor-hpgl0243.status
echo "###Finished ${PBS_JODID} hts_lmajor-hpgl0243.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
