...
#SBATCH --job-name=trst_hpgl0638_forward
#SBATCH --mem=1G
#SBATCH --cpus-per-task=1
#SBATCH --output=/fs/cbcb-lab/nelsayed/raw_data/ade/HPGL0638/processed/outputs/trst_hpgl0638_forward.sbatchout


echo "## Started /fs/cbcb-lab/nelsayed/raw_data/ade/HPGL0638/processed/scripts/06trst_hpgl0638_forward.sh at $(date) on $(hostname) with id ${SLURM_JOBID}." >> outputs/log.txt
...
echo $? > outputs/status/trst_hpgl0638_forward.status
echo "## Finished ${SLURM_JOBID} 06trst_hpgl0638_forward.sh at $(date), it took $(( SECONDS / 60 )) minutes." >> outputs/log.txt
