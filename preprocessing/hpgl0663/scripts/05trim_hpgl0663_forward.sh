...
#SBATCH --job-name=trim_hpgl0663_forward
#SBATCH --mem=40G
#SBATCH --cpus-per-task=3
#SBATCH --output=/fs/cbcb-lab/nelsayed/raw_data/ade/HPGL0663/processed/outputs/trim_hpgl0663_forward.sbatchout


echo "## Started /fs/cbcb-lab/nelsayed/raw_data/ade/HPGL0663/processed/scripts/05trim_hpgl0663_forward.sh at $(date) on $(hostname) with id ${SLURM_JOBID}." >> outputs/log.txt
cd /fs/cbcb-lab/nelsayed/raw_data/ade/HPGL0663/processed || exit

## This call to trimomatic removes illumina and epicentre adapters from hpgl0663_forward.fastq.xz:hpgl0663_reverse.fastq.xz.
...
if [[ ! -r "hpgl0663_forward.fastq.xz" ]]; then
  if [[ -r "sequences/hpgl0663_forward.fastq.xz" ]]; then
    mv sequences/hpgl0663_forward.fastq.xz . && pxz -d hpgl0663_forward.fastq.xz && pigz hpgl0663_forward.fastq &&\
       mv sequences/hpgl0663_reverse.fastq.xz . && pxz -d hpgl0663_reverse.fastq.xz && pigz hpgl0663_reverse.fastq
  else
    echo "Missing files. Did not find hpgl0663_forward.fastq.xz nor sequences/hpgl0663_forward.fastq.xz"
    exit 1
  fi
fi
...
trimmomatic PE \
  -threads 1 \
  -phred33 \
  <(less hpgl0663_forward.fastq.xz) <(less hpgl0663_reverse.fastq.xz) \
  hpgl0663_forward-trimmed_paired.fastq.gz hpgl0663_forward-trimmed_unpaired.fastq.gz \
  hpgl0663_reverse-trimmed_paired.fastq.gz hpgl0663_reverse-trimmed_unpaired.fastq.gz \
  ...
  1>outputs/hpgl0663-trimomatic.out 2>&1
...
ln -s hpgl0663_forward-trimmed.fastq.gz r1_trimmed.fastq.gz
ln -s hpgl0663_reverse-trimmed.fastq.gz r2_trimmed.fastq.gz

## The following lines give status codes and some logging
echo $? > outputs/status/trim_hpgl0663_forward.status
echo "## Finished ${SLURM_JOBID} 05trim_hpgl0663_forward.sh at $(date), it took $(( SECONDS / 60 )) minutes." >> outputs/log.txt
