...
#PBS -N fqc-hpgl0650 -l mem=6gb -l walltime=10:00:00 -l ncpus=8
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0650/outputs/qsub/fqc-hpgl0650.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0650/scripts/fqc-hpgl0650.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/fastqc && \
  fastqc --extract -o outputs/fastqc hpgl0650_forward.fastq.gz hpgl0650_reverse.fastq.gz \
  2>outputs/fastqc.out 1>&2

## The following lines give status codes and some logging
echo $? > outputs/status/fqc-hpgl0650.status
echo "###Finished ${PBS_JODID} fqc-hpgl0650.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
