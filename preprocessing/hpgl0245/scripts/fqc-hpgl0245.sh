...
#PBS -N fqc-hpgl0245 -l mem=6gb -l walltime=10:00:00 -l ncpus=8
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0245/outputs/qsub/fqc-hpgl0245.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0245/scripts/fqc-hpgl0245.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/fastqc && \
  fastqc --extract -o outputs/fastqc hpgl0245_forward-trimmed.fastq hpgl0245_reverse-trimmed.fastq \
  2>outputs/fastqc.out 1>&2

## The following lines give status codes and some logging
echo $? > outputs/status/fqc-hpgl0245.status
echo "###Finished ${PBS_JODID} fqc-hpgl0245.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
