...
#PBS -N th_linfantum-hpgl0242 -l mem=8gb -l walltime=18:00:00 -l ncpus=4
#PBS -o /cbcb/lab/nelsayed/raw_data/ade/HPGL0242/processed/outputs/qsub/th_linfantum-hpgl0242.qsubout -j oe -V -m n
echo "####Started /cbcb/lab/nelsayed/raw_data/ade/HPGL0242/processed/scripts/th_linfantum-hpgl0242.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/tophat_linfantum && tophat  -g 1  \
  -G /cbcbhomes/abelew/libraries/genome/linfantum.gff \
  --b2-very-sensitive -p 4 -o outputs/tophat_linfantum \
/cbcbhomes/abelew/libraries/genome/indexes/linfantum \
  hpgl0242_forward-trimmed.fastq.gz hpgl0242_reverse-trimmed.fastq.gz && \
  samtools sort -l 9 -n outputs/tophat_linfantum/accepted_hits.bam outputs/tophat_linfantum/accepted_sorted && \
  mv outputs/tophat_linfantum/accepted_sorted.bam outputs/tophat_linfantum/accepted_hits.bam && \
  samtools index outputs/tophat_linfantum/accepted_hits.bam && \
  samtools sort -l 9 -n outputs/tophat_linfantum/unmapped.bam outputs/tophat_linfantum/unmapped_sorted && \
  mv outputs/tophat_linfantum/unmapped_sorted.bam outputs/tophat_linfantum/unmapped.bam && \
  samtools index outputs/tophat_linfantum/unmapped.bam

## The following lines give status codes and some logging
echo $? > outputs/status/th_linfantum-hpgl0242.status
echo "###Finished ${PBS_JODID} th_linfantum-hpgl0242.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
