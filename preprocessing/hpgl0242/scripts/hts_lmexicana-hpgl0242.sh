...
#PBS -N hts_lmexicana-hpgl0242 -l mem=6gb -l walltime=10:00:00 -l ncpus=4
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0242/outputs/qsub/hts_lmexicana-hpgl0242.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0242/scripts/hts_lmexicana-hpgl0242.sh at $(date)" >> outputs/log.txt
...
htseq-count -q -f bam -s no  -i ID  \
  ...
  1>outputs/tophat_lmexicana/accepted_hits.count 2>outputs/tophat_lmexicana/accepted_hits.error && \
    xz -9e outputs/tophat_lmexicana/accepted_hits.count

## The following lines give status codes and some logging
echo $? > outputs/status/hts_lmexicana-hpgl0242.status
echo "###Finished ${PBS_JODID} hts_lmexicana-hpgl0242.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
