...
#PBS -N tpstats_hsapiens-hpgl0660 -l mem=1gb -l walltime=00:10:00 -l ncpus=1
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0660/outputs/qsub/tpstats_hsapiens-hpgl0660.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0660/scripts/tpstats_hsapiens-hpgl0660.sh at $(date)" >> outputs/log.txt
...
rpm=${rpm_tmp:-0}
stat_string=$(printf "hpgl0660-trimmed,hsapiens,%s,%s,%s,%s,%s,accepted_hits.count.xz" "${original_reads}" "${reads}" "${aligned}" "${failed}" "$rpm")
echo "$stat_string" >> outputs/tophat_stats.csv

## The following lines give status codes and some logging
echo $? > outputs/status/tpstats_hsapiens-hpgl0660.status
echo "###Finished ${PBS_JODID} tpstats_hsapiens-hpgl0660.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
