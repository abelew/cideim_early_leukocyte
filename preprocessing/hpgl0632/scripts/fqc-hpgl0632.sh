...
#PBS -N fqc-hpgl0632 -l mem=6gb -l walltime=10:00:00 -l ncpus=8
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0632/outputs/qsub/fqc-hpgl0632.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0632/scripts/fqc-hpgl0632.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/fastqc && \
  fastqc --extract -o outputs/fastqc hpgl0632_forward.fastq.gz hpgl0632_reverse.fastq.gz \
  2>outputs/fastqc.out 1>&2

## The following lines give status codes and some logging
echo $? > outputs/status/fqc-hpgl0632.status
echo "###Finished ${PBS_JODID} fqc-hpgl0632.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
