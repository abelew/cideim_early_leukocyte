...
#PBS -N fqc-hpgl0634 -l mem=6gb -l walltime=10:00:00 -l ncpus=8
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0634/outputs/qsub/fqc-hpgl0634.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/hpgl0634/scripts/fqc-hpgl0634.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/fastqc && \
  fastqc --extract -o outputs/fastqc hpgl0634_forward.fastq.gz hpgl0634_reverse.fastq.gz \
  2>outputs/fastqc.out 1>&2

## The following lines give status codes and some logging
echo $? > outputs/status/fqc-hpgl0634.status
echo "###Finished ${PBS_JODID} fqc-hpgl0634.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
