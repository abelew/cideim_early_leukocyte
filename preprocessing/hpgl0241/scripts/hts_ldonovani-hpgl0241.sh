...
#PBS -N hts_ldonovani-hpgl0241 -l mem=6gb -l walltime=10:00:00 -l ncpus=4
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0241/outputs/qsub/hts_ldonovani-hpgl0241.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0241/scripts/hts_ldonovani-hpgl0241.sh at $(date)" >> outputs/log.txt
cd /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0241 || exit

## Counting the number of hits in outputs/tophat_ldonovani/accepted_hits.bam for each feature found in /cbcbhomes/abelew/libraries/genome/ldonovani.gff
...
htseq-count -q -f bam -s no  -i ID  \
  outputs/tophat_ldonovani/accepted_hits.bam /cbcbhomes/abelew/libraries/genome/ldonovani.gff \
  1>outputs/tophat_ldonovani/accepted_hits.count 2>outputs/tophat_ldonovani/accepted_hits.error && \
    xz -9e outputs/tophat_ldonovani/accepted_hits.count

## The following lines give status codes and some logging
echo $? > outputs/status/hts_ldonovani-hpgl0241.status
echo "###Finished ${PBS_JODID} hts_ldonovani-hpgl0241.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
