...
#PBS -N kall_-HPGL0241 -l mem=6gb -l walltime=10:00:00 -l ncpus=4
#PBS -o /cbcb/lab/nelsayed/raw_data/ade/HPGL0241/processed/outputs/qsub/kall_-HPGL0241.qsubout -j oe -V -m n
echo "####Started /cbcb/lab/nelsayed/raw_data/ade/HPGL0241/processed/scripts/kall_-HPGL0241.sh at $(date)" >> outputs/log.txt
...
mkdir -p outputs/kallisto_ && sleep 10 && \
kallisto quant --plaintext -t 4 -b 100 -o outputs/kallisto_ -i /cbcbhomes/abelew/libraries/genome/indexes/hsapiens.idx \
   2>outputs/kallisto/HPGL0241_R1_filtered_adaptertrim.fastq.xz:HPGL0241_R2_filtered_adaptertrim-kallisto.err 1>outputs/kallisto/HPGL0241_R1_filtered_adaptertrim.fastq.xz:HPGL0241_R2_filtered_adaptertrim-kallisto.out

## The following lines give status codes and some logging
echo $? > outputs/status/kall_-HPGL0241.status
echo "###Finished ${PBS_JODID} kall_-HPGL0241.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
