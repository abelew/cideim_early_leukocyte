...
#PBS -N th_hsapiens-hpgl0248 -l mem=20gb -l walltime=144:00:00 -l ncpus=4
#PBS -o /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0248/outputs/qsub/th_hsapiens-hpgl0248.qsubout -j oe -V -m n
echo "####Started /cbcb/nelsayed-scratch/atb/rnaseq_lpanamensis/preprocessing/old/hpgl0248/scripts/th_hsapiens-hpgl0248.sh at $(date)" >> outputs/log.txt
...
/cbcbhomes/abelew/libraries/genome/indexes/hsapiens \
  hpgl0248_forward-trimmed.fastq.gz hpgl0248_reverse-trimmed.fastq.gz && \
  ...
  samtools sort -l 9 -n outputs/tophat_hsapiens/unmapped.bam outputs/tophat_hsapiens/unmapped_sorted && \
  mv outputs/tophat_hsapiens/unmapped_sorted.bam outputs/tophat_hsapiens/unmapped.bam && \
  samtools index outputs/tophat_hsapiens/unmapped.bam

## The following lines give status codes and some logging
echo $? > outputs/status/th_hsapiens-hpgl0248.status
echo "###Finished ${PBS_JODID} th_hsapiens-hpgl0248.sh at $(date), it took $(( $SECONDS / 60 )) minutes." >> outputs/log.txt
