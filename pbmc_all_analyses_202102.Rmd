...
Oh yeah, unlike the macrophage analysis sheet, in this one I actually
specified the tritrypdb version, so it should actually be consistent.
...
hs_cds_pbmc <- create_expt(
  ...
  exclude_genes_expt(method="keep",
                     column="gene_biotype",
                     patterns="protein_coding")
hs_cds_inf <- create_expt(
...
## Finally, set the sample names to make figures look nice
pbmc_newnames <- pData(hs_cds_pbmc)[["figurelabel"]]
hs_cds_pbmc <- set_expt_samplenames(hs_cds_pbmc, newnames=pbmc_newnames)
inf_newnames <- pData(hs_cds_inf)[["figurelabel"]]
...
shortnames <- pData(hs_cds_pbmc)[["label"]]
plt <- plot_pca(pbmc_norm, label_size=5, plot_labels=FALSE)$plot +
                geom_text_repel(check_overlap=TRUE, aes(label=shortnames),
                                max.overlaps = 100,
                                size=6, box.padding=ggplot2::unit(0.5, "lines"),
                                point.padding = ggplot2::unit(1.6, "lines"),
                                arrow = ggplot2::arrow(length = ggplot2::unit(0.01, "npc")))

pp(file="figures/fig_2a.pdf", image=plt)
plt
...
shortnames <- pData(hs_minus)[["label"]]
minus_norm <- normalize_expt(hs_minus, convert="cpm", norm="quant", filter=TRUE, transform="log2")
plt <- plot_pca(minus_norm, plot_labels=FALSE)$plot +
                geom_text_repel(check_overlap=TRUE, aes(label=shortnames),
                                max.overlaps = 100,
                                size=6, box.padding=ggplot2::unit(0.5, "lines"),
                                point.padding = ggplot2::unit(1.6, "lines"),
                                arrow = ggplot2::arrow(length = ggplot2::unit(0.01, "npc")))
pp(file="figures/fig_2b.pdf", image=plt)
plt

## Step 2a
##minus_nb <- normalize_expt(hs_minus, transform="log2", convert="cpm", filter=TRUE, batch="svaseq")
minus_nb <- normalize_expt(hs_minus, convert="cpm", filter=TRUE, batch="svaseq", transform="log2")
plt <- plot_pca(minus_nb, plot_labels=FALSE)$plot +
                geom_text_repel(check_overlap=TRUE, aes(label=shortnames),
                                max.overlaps = 100,
                                size=6, box.padding=ggplot2::unit(0.5, "lines"),
                                point.padding = ggplot2::unit(1.6, "lines"),
                                arrow = ggplot2::arrow(length = ggplot2::unit(0.01, "npc")))
pp(file="figures/fig_2c.pdf", image=plt)
plt
...
pp(file="figures/pbmc_s1_libsize.pdf", image=pbmc_s1_libsize$plot)

pbmc_heat_norm <- normalize_expt(hs_cds_pbmc, filter=TRUE, batch="svaseq",
                                 convert="cpm", transform="log2")
pbmc_s1_heat <- plot_disheat(pbmc_heat_norm, cexRow=1.5, cexCol=1.5)
pp(file="figures/pbmc_s1_disheat.pdf", image=pbmc_s1_heat)
```
...
test_up$pvalue_plots$bpp_plot_over
...
pp(file="images/upset.pdf", image=plt)
```
...
```{r msig_immunology}
broad_c7 <- GSEABase::getGmt("reference/msigdb_v7.2/c7.all.v7.2.entrez.gmt.xz",
                             collectionType=GSEABase::BroadCollection(category="c7"),
                             geneIdType=GSEABase::EntrezIdentifier())

broad_c8 <- GSEABase::getGmt("reference/msigdb_v7.2/c8.all.v7.2.entrez.gmt.xz",
                             collectionType=GSEABase::BroadCollection(category="c8"),
                             geneIdType=GSEABase::EntrezIdentifier())

broad_c2 <- GSEABase::getGmt("reference/msigdb_v7.2/c2.all.v7.2.entrez.gmt.xz",
...
pbmc_scored <- get_sig_gsva_categories(pbmc_gsva, factor="chr")
